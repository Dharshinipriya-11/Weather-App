...
const WeatherDetails = ({ children }) => {
  ...
  const [fetchingFromLocation, setFetchingFromLocation] = useState(false);
  ...
  const fetchData = async (fromLocation = false) => {
    setFetchingFromLocation(fromLocation);
    setError("");  
    try {
  ...
  return (
    <MyContext.Provider value={{ city, setCity, data, setData, error,setError, forecastdata, handleClick,setForecastData, fetchData, fetchForecast,more,setMore }}>
      {children}

      {!fetchingFromLocation && error && ( 
        <div className="iframe-style">
          <p className="error-text">{error}⚠️</p>
        </div>
      )}
